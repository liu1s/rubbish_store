...
class Util_Nlog_FluentLogger extends BaseLogger {
     ...
     * @return FluentLogger
     */
    public function __construct($host = self::DEFAULT_ADDRESS, $port = self::DEFAULT_LISTEN_PORT, array $options = array()) {
     ...
     */
    public static function open($host = self::DEFAULT_ADDRESS, $port = self::DEFAULT_LISTEN_PORT, $options = array()) {
    ...
    protected function connect() {
        ...
        $socket = @stream_socket_client($this->transport, $errno, $errstr, $this->getOption("connection_timeout", self::CONNECTION_TIMEOUT), $connect_options);
    ...
    public function post($tag, $data) {
        if (is_array($data)) {
            $entity = array($tag, time(), $data);
        } else {
            $entity = array($tag, time(), array('all' => $data));
        }
